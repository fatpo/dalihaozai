...
public class ActivityController {
    ...
    @PostMapping("/sync")
    public ReturnObj<Void> syncActivity(@RequestBody Activity activity, BindingResult bindingResult) {
        log.info("活动同步入参：{}", activity);
        if (null != bindingResult && bindingResult.hasErrors()) {
            List<String> msg = bindingResult.getAllErrors().stream()
                .map(ObjectError::getDefaultMessage)
                .collect(Collectors.toList());
            log.error("同步活动异常：{}", msg);
            return new ReturnObj<>(ReturnCode.PARAM_ERROR);
        }
        try {
            activityService.syncActivity(activity);
            return new ReturnObj<>(ReturnCode.SUCCESS);
        }catch (Exception ex){
            log.error("同步活动异常 {} ", activity, ex);
